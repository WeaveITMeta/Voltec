...
Product: VSupreme  |  Component: ThighR
Run: blender --background --python this_script.py

Right thigh — mirror of ThighL. Heavy-duty armored tube with hip/knee
joint housings, cycloidal drive motor bulge, segmented armor.
"""
...
HIP_JOINT_R=0.055; HIP_JOINT_H=0.030; KNEE_JOINT_R=0.050; KNEE_JOINT_H=0.025
...
def create_geometry():
    ...
    sol=t.modifiers.new("Shell",'SOLIDIFY'); sol.thickness=WALL; sol.offset=-1
    bpy.context.view_layer.objects.active=t; bpy.ops.object.modifier_apply(modifier="Shell")
    ...
    bool_op(jt,jti,'DIFFERENCE'); bool_op(t,jt,'UNION')
    ...
    bool_op(jb,jbi,'DIFFERENCE'); bool_op(t,jb,'UNION')
    # Motor bulge mirrored to right (positive X)
    bulge=add_cyl(MOTOR_BULGE_R,MOTOR_BULGE_H,24,loc=(TOP_R*0.6,LENGTH/6,0))
    bool_op(t,bulge,'UNION')
    for y in [-LENGTH/8, LENGTH/8]:
        g=add_cube(TOP_R*2.5,0.002,TOP_R*2.5,loc=(0,y,0)); bool_op(t,g,'DIFFERENCE')
    ch=add_cube(0.006,LENGTH*0.6,WALL+0.005,loc=(-TOP_R*0.5,0,-TOP_R*0.3))
    bool_op(t,ch,'DIFFERENCE')
    t.name=f"{PRODUCT}_{COMPONENT}"; t.data.name=f"{PRODUCT}_{COMPONENT}_mesh"; return t
