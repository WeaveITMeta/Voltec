...
Product: VSupreme  |  Component: GauntletL
Run: blender --background --python this_script.py

Left gauntlet — 5-DOF articulated hand shell with knuckle guards,
finger channels, palm plate, haptic mesh cavity, and wrist coupling ring.
"""
...
def create_geometry():
    ...
    # 4 finger tubes + 1 thumb
    finger_offsets = [(-0.030, 0), (-0.010, 0), (0.010, 0), (0.030, 0)]
    for fx, fz in finger_offsets:
        finger = add_cyl(FINGER_R, FINGER_H, 16,
                        loc=(fx, -PALM_H/2 - FINGER_H/2, fz))
        bool_op(palm, finger, 'UNION')
        # Knuckle guard — sphere at base
    ...
    bool_op(wr, wri, 'DIFFERENCE')
    bool_op(palm, wr, 'UNION')
