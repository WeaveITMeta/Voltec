...
Product: VSupreme  |  Component: PauldronR
Run: blender --background --python this_script.py

Right shoulder pauldron — mirror of PauldronL. Curved armor shell over
3-DOF shoulder actuator housing with hardpoint and mounting flange.
"""
...
OUTER_RX = 0.100; OUTER_RY = 0.090; OUTER_RZ = 0.110
WALL = 0.004; HARDPOINT_R = 0.018; HARDPOINT_H = 0.012
MOUNT_R = 0.030; MOUNT_H = 0.008
...
def create_geometry():
    ...
    p = bpy.context.active_object; p.scale = (OUTER_RX, OUTER_RY, OUTER_RZ)
    bpy.ops.object.transform_apply(scale=True)
    sol = p.modifiers.new("Shell", 'SOLIDIFY'); sol.thickness = WALL; sol.offset = -1
    bpy.context.view_layer.objects.active = p; bpy.ops.object.modifier_apply(modifier="Shell")
    cut = add_cube(0.3, OUTER_RY, 0.3, loc=(0, -OUTER_RY/2, 0))
    bool_op(p, cut, 'DIFFERENCE')
    # Mirror: cut inner side on negative X (toward torso)
    ...
    bool_op(p, hp, 'UNION')
    flange = add_cyl(MOUNT_R, MOUNT_H, 32, loc=(-OUTER_RX * 0.6, 0.02, 0), rot=(0, 0, math.radians(90)))
    bool_op(p, flange, 'UNION')
    for i in range(3):
        sl = add_cube(0.004, 0.025, OUTER_RZ * 0.5, loc=(OUTER_RX * 0.5, 0.03 + i * 0.015, 0))
        bool_op(p, sl, 'DIFFERENCE')
    p.name = f"{PRODUCT}_{COMPONENT}"; p.data.name = f"{PRODUCT}_{COMPONENT}_mesh"
    return p
