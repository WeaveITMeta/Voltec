...
Product: VSupreme  |  Component: ForearmR
Run: blender --background --python this_script.py

Right forearm — armored tube with directed energy pulse (DEP) emitter housing,
lens port, cooling fins, wrist joint housing.
"""
...
def create_geometry():
    ...
    # DEP emitter lens port — circular boss on underside with recessed lens
    lens_boss=add_cyl(LENS_R+0.005, LENS_H+0.010, 24, loc=(0, -LENGTH/4, -BOT_R*0.8))
    lens_recess=add_cyl(LENS_R, LENS_H, 24, loc=(0, -LENGTH/4, -BOT_R*0.8))
    bool_op(lens_boss, lens_recess, 'DIFFERENCE')
    bool_op(arm, lens_boss, 'UNION')

    # Cooling fins around DEP housing — 4 thin radial fins
    for angle in [0, 90, 180, 270]:
        rad = math.radians(angle)
        fx = LENS_R * 1.5 * math.cos(rad)
        fz = -BOT_R*0.8 + LENS_R * 1.5 * math.sin(rad)
        fin = add_cube(0.002, 0.025, 0.010, loc=(fx, -LENGTH/4, fz))
        bool_op(arm, fin, 'UNION')
