...
Product: V-Core  |  Component: ElectricalConverter
Material: SiC inverter + transformer — DC to 240VAC split-phase
Run: blender --background --python this_script.py
"""
import bpy, bmesh, math, os

PRODUCT = "VCore"
COMPONENT = "ElectricalConverter"
MATERIAL = "SiC"
...
def create_geometry():
    ...
    for i in range(8):
        x = -BOX_W/2.0 + 0.020 + i * 0.030
        bpy.ops.mesh.primitive_cube_add(size=1.0, location=(x, BOX_H/2.0 + 0.012, 0))
        fin = bpy.context.active_object
        fin.scale = (0.002, 0.012, BOX_D/2.0 - 0.010)
        bpy.ops.object.transform_apply(scale=True)
        bool_op(box, fin, 'UNION')

    # AC output conduit stub
    bpy.ops.mesh.primitive_cylinder_add(vertices=16, radius=0.015, depth=0.025,
                                         location=(BOX_W/2.0 + 0.0125, 0, 0))
    conduit = bpy.context.active_object
    conduit.rotation_euler = (0, 0, math.radians(90))
    bpy.ops.object.transform_apply(rotation=True)
    bool_op(box, conduit, 'UNION')

    # DC input connector
    bpy.ops.mesh.primitive_cylinder_add(vertices=12, radius=0.010, depth=0.020,
                                         location=(-BOX_W/2.0 - 0.010, 0, 0))
    dc_in = bpy.context.active_object
    dc_in.rotation_euler = (0, 0, math.radians(90))
    bpy.ops.object.transform_apply(rotation=True)
    bool_op(box, dc_in, 'UNION')
