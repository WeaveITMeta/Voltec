...
Product: V-Core  |  Component: HotSideHX
Material: Inconel 718 — heat pipe condenser to Stirling hot-side interface
...
# Disk-shaped heat exchanger: Ø250mm × 60mm thick
HX_R = 0.125
HX_H = 0.060
WALL = 0.004

PBR = {
    "base_color": (0.65, 0.65, 0.60, 1.0),
    "metallic": 1.0,
    "roughness": 0.30,
    "alpha": 1.0,
...
def create_geometry():
    # Main disk body
    bpy.ops.mesh.primitive_cylinder_add(vertices=48, radius=HX_R, depth=HX_H, location=(0, 0, 0))
    hx = bpy.context.active_object

    # Hollow interior for flow channels
    sol = hx.modifiers.new("Shell", 'SOLIDIFY')
    sol.thickness = WALL
    sol.offset = -1
    bpy.context.view_layer.objects.active = hx
    bpy.ops.object.modifier_apply(modifier="Shell")

    # 12 heat pipe stub receptacles on bottom face
    for i in range(12):
        angle = math.radians(i * 30)
        x = 0.060 * math.cos(angle)
        z = 0.060 * math.sin(angle)
        bpy.ops.mesh.primitive_cylinder_add(vertices=12, radius=0.010, depth=0.020,
                                             location=(x, -HX_H/2.0 - 0.010, z))
        stub = bpy.context.active_object
        bool_op(hx, stub, 'UNION')

    # Two outlet ports on top for Stirling engine connection
    for x_off in [-0.06, 0.06]:
        bpy.ops.mesh.primitive_cylinder_add(vertices=16, radius=0.025, depth=0.025,
                                             location=(x_off, HX_H/2.0 + 0.0125, 0))
        port = bpy.context.active_object
        bool_op(hx, port, 'UNION')

    hx.name = f"{PRODUCT}_{COMPONENT}"
    hx.data.name = f"{PRODUCT}_{COMPONENT}_mesh"
    return hx
