...
Product: V-Core  |  Component: ColdRadiator
Material: Aluminum 6061-T6 + copper tubing — finned radiator for heat rejection
Run: blender --background --python this_script.py
"""
import bpy, bmesh, math, os

PRODUCT = "VCore"
COMPONENT = "ColdRadiator"
MATERIAL = "Al6061T6"
OUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
OUT_FILE = f"{PRODUCT}_{COMPONENT}.glb"

# Rectangular radiator block with fins: 400×300×100mm
RAD_W = 0.400
RAD_H = 0.100
RAD_D = 0.300
FIN_COUNT = 15
...
PBR = {
    "base_color": (0.75, 0.78, 0.80, 1.0),  # Brushed aluminum
    "metallic": 1.0,
    "roughness": 0.35,
    "alpha": 1.0,
...
def create_geometry():
    ...
    radiator.scale = (RAD_W/2.0, (RAD_H - FIN_HEIGHT)/2.0, RAD_D/2.0)
    bpy.ops.object.transform_apply(scale=True)

    # Cooling fins
    spacing = RAD_D / (FIN_COUNT + 1)
    for i in range(FIN_COUNT):
        z_pos = -RAD_D/2.0 + spacing * (i + 1)
        bpy.ops.mesh.primitive_cube_add(size=1.0, location=(0, -(RAD_H - FIN_HEIGHT)/2.0 - FIN_HEIGHT/2.0, z_pos))
        fin = bpy.context.active_object
        fin.scale = (RAD_W/2.0 - 0.010, FIN_HEIGHT/2.0, FIN_THICKNESS/2.0)
        bpy.ops.object.transform_apply(scale=True)
        bool_op(radiator, fin, 'UNION')

    # Inlet/outlet pipe stubs on top
    for x_off in [-0.10, 0.10]:
        bpy.ops.mesh.primitive_cylinder_add(vertices=16, radius=0.015, depth=0.030,
                                             location=(x_off, (RAD_H - FIN_HEIGHT)/2.0 + 0.015, 0))
        pipe = bpy.context.active_object
        bool_op(radiator, pipe, 'UNION')

    radiator.name = f"{PRODUCT}_{COMPONENT}"
    radiator.data.name = f"{PRODUCT}_{COMPONENT}_mesh"
    return radiator
